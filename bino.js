(function(prot) {
  ...
  (this.bino = function(dwords, bitlength) {
    return new construct(dwords === void 0 ? [] : [].concat(dwords), bitlength);
  }).prototype = construct.prototype = prot;
  this.bino.chunk = function (str, width, delimiter){
    return ('' + str).replace(new RegExp('([^]{' + width + '})(?!$)', 'g'), "$1" + delimiter);
  };
})({
  //bino prototype
  ...
  fromHex: function(hex, bits) {
    ...
    while (pos < len) {
      ...
      if ((c > 47 && c < 58) || (c > 64 && c < 71) || (c > 96 && c < 103)) {
        bin[parsedBits >>> 5] ^=
          ((c > 64 ? c + 9 : c) & 15) << (28 - (parsedBits & 31));
        parsedBits += 4;
      }
    }
  ...
  toHex: function(chunkSize, delimiter) {
    ...
    if ((bits & 3) === 0) {
      var hex = "",
        ...
        len = data.length;
      while (pos < len) {
        hex += (data[pos++] + 0xf00000000).toString(16).slice(1)
      }
  ...
  fromText: function(txt) {
    ...
    for (len = txt.length; pos < len; pos++, bits += 8) {
      bin[bits >>> 5] ^= (txt.charCodeAt(pos) & 0xff) << (24 - (bits & 31));
    }
  ...
  toText: function() {
    ...
    while (pos < bits) {
      txt += toChar((data[pos >>> 5] >>> (24 - (pos & 31))) & 0xff);
      pos += 8;
    }
    try{
  ...
  toBase64: function(chunkSize, delimiter) {
    ...
    base64 += toChar(
      value + 71 - (
        ...
        if((charpos & 3) ^ 3){
          buffer = (buffer << 8) ^ ((data[bitpos >>> 5] >>> (24 - (bitpos & 31))) & 0xff);
          bitpos += 8;
        }
      }
  ...
  fromBase64: function(b64) {
    ...
    while(charpos < len){
      ...
      if(value < 64){
        ...
        if((++count & 3) ^ 1){
          bin[bits >>> 5] ^= 
            ((buffer >> (4 - 2 * ((count - 2) & 3))) & 255) << (24 - (bits & 31));
          bits += 8;
        }
      }
    }
